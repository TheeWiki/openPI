...
/**
 * TODO: Respawning, obstacles, food
 * 
 * @TODO: other weapon wielding
 * @author Dennis
 */
public class CastleWars {
	 ...
	 * Game timers.
	 */
	private static final int GAME_TIMER = 1500 * 600; // 1500 * 600 = 900000ms =
														// 15 minutes
	private static final int GAME_START_TIMER = 12;
	/*
	...
	private static int timeRemaining = 3;
	...
	public static void toWaitingRoom(Client player, int team) {
		if (team == 1) {
			...
			if (getZammyPlayers() >= getSaraPlayers() || getSaraPlayers() == 0) {
				...
				player.getPA().movePlayer(WAIT_ROOM[team - 1][0] + Misc.random(5),
	...
	public static void createHintIcon(Client player, int t) {
		...
		while (iterator.hasNext()) {
			...
			if (gameRoom.get(teamPlayer) == t) {
				System.out.println(
						"created hint icons for playername " + teamPlayer.playerName + " and team number: " + t);
		...
		while (iterator.hasNext()) {
			...
			if (player != null) {
				...
				player.getPA().sendFrame126(
						"Next Game Begins In:@red@ " + ((gameStartTimer * 3) + (timeRemaining * 3)) + " @whi@seconds.",
						6570);
	...
	public static void updateInGamePlayers() {
		if (getSaraPlayers() > 0 && getZammyPlayers() > 0) {
			...
			while (iterator.hasNext()) {
				...
				// player.getPA().showInterface(11146);
	...
	public static void startGame() {
		...
		while (iterator.hasNext()) {
			...
			player.getPA().movePlayer(GAME_ROOM[team - 1][0] + Misc.random(3), GAME_ROOM[team - 1][1] - Misc.random(3),
					1);
			player.getPA().movePlayer(GAME_ROOM[team - 1][0] + Misc.random(3), GAME_ROOM[team - 1][1] - Misc.random(3),
					1);
	...
	public static void removePlayerFromCw(Client player) {
		...
		if (gameRoom.containsKey(player)) {
			 ...
			 */
			if (player.getItems().playerHasItem(SARA_BANNER)) {
				player.getItems().removeItem(player.playerEquipment[3], 3);
				setSaraFlag(0); // safe flag
			} else if (player.getItems().playerHasItem(ZAMMY_BANNER)) {
				player.getItems().removeItem(player.playerEquipment[3], 3);
	...
	public static int getZammyPlayers() {
		int players = 0;
		Iterator<Integer> iterator = (!waitingRoom.isEmpty()) ? waitingRoom.values().iterator()
				: gameRoom.values().iterator();
		while (iterator.hasNext()) {
