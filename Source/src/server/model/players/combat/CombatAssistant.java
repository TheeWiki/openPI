			...
			if (c.attackTimer <= 0) {
				...
				if (c.usingSpecial && !c.usingMagic) {
					if (checkSpecAmount(c.getEquipment()[c.playerWeapon])) {
						...
						activateSpecial(c.getEquipment()[c.playerWeapon], i);
						return;
					} else {
...
//			}
	...
	public void activateSpecial(int weapon, int i) {
		...
		case 1305: // dragon long
			c.playGraphic(Graphic.create(248, 100));
			c.playAnimation(Animation.create(1058));
			c.specAccuracy = 1.10;
			c.specDamage = 1.20;
			if (c.playerIndex > 0)
				Experience.calculateDamage(c, (Client) PlayerHandler.players[c.playerIndex], 0);
			else if (c.npcIndex > 0)
				Experience.calculateDamageNPC(c, c.npcIndex, 0);
			c.hitDelay = getHitDelay(c.getItems().getItemName(c.getEquipment()[c.playerWeapon]).toLowerCase());
			break;
		...
		case 5680:
		case 5698:
			c.specAccuracy = 1.25D;
			c.specDamage = 1.15D;
			c.playGraphic(Graphic.create(252, 0, 100));
			c.playAnimation(Animation.create(1062));
			c.doubleHit = true;
			if (c.playerIndex > 0)
				Experience.calculateDamage(c, (Client) PlayerHandler.players[c.playerIndex], 0);
			else if (c.npcIndex > 0)
		...
		case 11730:
			c.playGraphic(Graphic.create(1224, 0, 100));
			c.playAnimation(Animation.create(811));
			c.doubleHit = true;
			c.ssSpec = true;
			c.specAccuracy = 1.30;
			if (c.playerIndex > 0)
		...
		case 4151: // whip
			...
			c.specAccuracy = 1.10;
			c.playAnimation(Animation.create(1658));
			if (c.playerIndex > 0)
				Experience.calculateDamage(c, (Client) PlayerHandler.players[c.playerIndex], 0);
			else if (c.npcIndex > 0)
				Experience.calculateDamageNPC(c, c.npcIndex, 0);
		...
		case 13092:
			c.playAnimation(Animation.create(440));
			c.playGraphic(Graphic.create(1234, 0, 100));
			c.specAccuracy = 1.10;
			c.specDamage = 1.20;
			if (c.playerIndex > 0)
				Experience.calculateDamage(c, (Client) PlayerHandler.players[c.playerIndex], 0);
		...
		case 12006:
			c.playAnimation(Animation.create(1658));
			if (c.playerIndex > 0) {
				PlayerHandler.players[i].playGraphic(Graphic.create(341, 0, 100));
				PlayerHandler.players[i].freezeTimer = 15;
				PlayerHandler.players[i].stopMovement();
				if (Misc.random(1) == 0) {
					if (PlayerHandler.players[i].poisonDamage <= 0) {
						((Client) PlayerHandler.players[i]).getPA().appendPoison(13);
					}
				}
			}
			c.specAccuracy = 1.10;
		...
		case 11802: // ags
			c.playAnimation(Animation.create(7061));
			c.specDamage = 1.3500;
			c.specAccuracy = 1.95;
			c.playGraphic(Graphic.create(1211, 0, 0));
			if (c.playerIndex > 0)
				Experience.calculateDamage(c, (Client) PlayerHandler.players[c.playerIndex], 0);
			else if (c.npcIndex > 0)
				Experience.calculateDamageNPC(c, c.npcIndex, 0);
		...
		case 11700:
			c.playAnimation(Animation.create(4302));
			c.playGraphic(Graphic.create(1221, 0, 0));
			c.specAccuracy = 1.25;
			c.specEffect = 2;
			if (c.playerIndex > 0)
				Experience.calculateDamage(c, (Client) PlayerHandler.players[c.playerIndex], 0);
		...
		case 11696:
			c.playAnimation(Animation.create(4301));
			c.playGraphic(Graphic.create(1223, 0, 0));
			c.specDamage = 1.10;
			c.specAccuracy = 1.5;
			c.specEffect = 3;
			if (c.playerIndex > 0)
		...
		case 11698:
			c.playAnimation(Animation.create(4303));
			c.playGraphic(Graphic.create(1220, 0, 0));
			c.specAccuracy = 1.25;
			c.specEffect = 4;
			if (c.playerIndex > 0)
				Experience.calculateDamage(c, (Client) PlayerHandler.players[c.playerIndex], 0);
		...
		case 1249:
			c.playAnimation(Animation.create(405));
			c.playGraphic(Graphic.create(253, 0, 100));
			if (c.playerIndex > 0) {
				if (c.inDuelArena()) {
					c.sendMessage("You can't use this special attack in duel arena.");
					return;
				}
				Client o = (Client) PlayerHandler.players[i];
		...
		case 3204: // d hally
			c.playGraphic(Graphic.create(282, 0, 100));
			c.playAnimation(Animation.create(1203));
			if (c.playerIndex > 0)
				Experience.calculateDamage(c, (Client) PlayerHandler.players[c.playerIndex], 0);
			else if (c.npcIndex > 0)
				Experience.calculateDamageNPC(c, c.npcIndex, 0);
		...
		case 4153: // maul
			this.c.playAnimation(Animation.create(1667));
			this.c.playGraphic(Graphic.create(340, 0, 100));
			if (c.playerIndex > 0) {
				Experience.calculateDamage(c, (Client) PlayerHandler.players[c.playerIndex], 0);
				this.c.getCombat().appendDamage(c.playerIndex, 0, 1);
			}
			if (c.npcIndex > 0) {
				this.c.getCombat().appendDamageNPC(i, 0, 1);
			}
			break;

		case 4587: // dscimmy
			c.playGraphic(Graphic.create(347, 0, 100));
			c.specEffect = 1;
			c.playAnimation(Animation.create(1872));
			if (c.playerIndex > 0)
				Experience.calculateDamage(c, (Client) PlayerHandler.players[c.playerIndex], 0);
			else if (c.npcIndex > 0)
				Experience.calculateDamageNPC(c, c.npcIndex, 0);
		...
		case 1434: // mace
			c.playAnimation(Animation.create(1060));
			c.playGraphic(Graphic.create(251, 0, 100));
			...
			if (c.playerIndex > 0)
				...
				Experience.calculateDamageNPC(c, c.npcIndex, 0);
			c.hitDelay = getHitDelay(c.getItems().getItemName(c.getEquipment()[c.playerWeapon]).toLowerCase()) + 1;
			break;
		...
		case 859: // magic long
			...
			c.playAnimation(Animation.create(426));
			c.playGraphic(Graphic.create(250, 0, 100));
			c.hitDelay = getHitDelay(c.getItems().getItemName(c.getEquipment()[c.playerWeapon]).toLowerCase());
			c.projectileStage = 1;
			if (c.fightMode == 2)
				c.attackTimer--;
		...
		case 861: // magic short
			...
			c.getItems().deleteArrow();
			c.lastWeaponUsed = weapon;
			c.playGraphic(Graphic.create(256, 0, 100));
			CycleEventHandler.getSingleton().addEvent(0, c, new CycleEvent() {
			...
			c.playAnimation(Animation.create(1074));
			c.projectileStage = 1;
			if (c.fightMode == 2)
				c.attackTimer--;
			if (c.playerIndex > 0) {
				c.dbowIndex = c.playerIndex;
				c.dbowTimer = 4;
				Experience.calculateDamage(c, (Client) PlayerHandler.players[c.playerIndex], 1);
				fireProjectilePlayer();
			} else if (c.npcIndex > 0) {
				c.dbowIndex1 = c.npcIndex;
				c.dbowTimer = 5;
				Experience.calculateDamageNPC(c, c.npcIndex, 1);
				fireProjectileNpc();
			}
			break;
