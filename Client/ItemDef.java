...
public final class ItemDef {
	...
	/*
	 * public static void dumpStacks() { try { FileOutputStream out = new
	 * FileOutputStream(new File("stackable.dat")); for (int j = 0; j < 12000;
	 * j++) { ItemDef item = ItemDef.forID(j); out.write(item.stackable ? 1 :
	 * 0); } out.write(-1); out.close(); } catch (IOException ioe)
	 * {ioe.printStackTrace();} }
	 * 
	 * public static void dumpNotes() { try { FileOutputStream out = new
	 * FileOutputStream(new File("notes.dat")); for (int j = 0; j < 12000; j++)
	 * { ItemDef item = ItemDef.forID(j); out.write(item.certTemplateID != -1 ?
	 * 0 : 1); } out.write(-1); out.close(); } catch (IOException ioe)
	 * {ioe.printStackTrace();}
	 * 
	 * 
	 * }
	...
	public static ItemDef forID(int i) {
		...
		case 11722:
			...
			break;
		case 5641:
			itemDef.name = "Death-touch Dart";
			itemDef.description = "Touched by no other than Death himself, or herself...it".getBytes();
			break;
